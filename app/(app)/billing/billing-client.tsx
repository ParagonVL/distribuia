  ...
  const handleUpgrade = async (plan: "starter" | "pro") => {
    ...
    try {
      const response = await fetch("/api/stripe/create-checkout", {
        ...
        body: JSON.stringify({ priceId: PLAN_PRICES[plan].priceId, waiverAccepted: waiverAccepted || hasStripeSubscription }),
      });
      ...
      {successVisible && (
        <div className="mb-6 p-4 bg-success/10 border border-success/30 rounded-lg">
          <div className="flex items-start gap-3">
            <svg aria-hidden="true" className="w-5 h-5 text-success flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
